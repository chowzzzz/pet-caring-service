...
function initRouter(app) {
  ...
  app.get("/caretaker-activate", passport.authMiddleware(), passport.verifyNotAdmin(), caretakerActivate);
...
function caretakerActivate(req, res, next) {
  const username = req.user.username;
  const isCaretaker = req.isAuthenticated() ? req.user.isCaretaker : false;

  if (!isCaretaker) {
    pool.query(sql_query.query.caretaker_activate, [username], (err) => {
      if (err) {
        console.error(err);
      }
    });
	...
	pool.query(sql_query.query.get_user, [username], (err, details) => {
		...
		pool.query(sql_query.query.fulltime_leavedays, [username], (err, leaves) => {
			...
			res.render("caretaker-leaves", {
				...
				details: details.rows,
				leaves: leaves.rows,
				isFulltime: req.isAuthenticated(),
				isSignedIn: req.isAuthenticated(),
				isAdmin: req.isAuthenticated() ? req.user.userType == "Admin" : false
			});
		});
